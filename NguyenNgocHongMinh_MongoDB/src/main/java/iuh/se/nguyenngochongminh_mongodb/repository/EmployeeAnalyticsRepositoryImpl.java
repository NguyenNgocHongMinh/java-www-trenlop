...
public class EmployeeAnalyticsRepositoryImpl implements EmployeeAnalyticsRepository {
    ...
    public List<AvgSalaryByDeptIdDTO> getCountAndAvgSalaryByDept() {
        Aggregation agg = newAggregation(
                group("deptId").count().as("count").avg("salary").as("avgSalary"),
                project("count", "avgSalary").and("_id").as("deptId")
        );
        return mongoTemplate.aggregate(agg, "employees", AvgSalaryByDeptIdDTO.class).getMappedResults();
    }

    @Override
    public List<AvgAgeByStatusDTO> getCountAndAvgAgeByStatus() {
        Aggregation agg = newAggregation(
                group("status").count().as("count").avg("age").as("avgAge"),
                project("count", "avgAge").and("_id").as("status")
        );
        return mongoTemplate.aggregate(agg, "employees", AvgAgeByStatusDTO.class).getMappedResults();
    }
    ...
    public List<Document> findAllMaxSalaryEmployees() {
        ...
        return mongoTemplate.aggregate(agg, "employees", Document.class).getMappedResults();
    }
